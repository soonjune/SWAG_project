...
from qlearn.toys.model import SWAG
...
class SwagAgent():
    def __init__(self, args, env):
        ...
        self.online_net = SWAG(args, self.action_space)
        if args.model and os.path.isfile(args.model):
            self.online_net.load_state_dict(torch.load(args.model))
        self.online_net.train()

        self.target_net = SWAG(args, self.action_space)
        self.update_target_net()
        self.target_net.eval()
        for param in self.target_net.parameters():
            param.requires_grad = False

        self.optimiser = optim.SGD(self.online_net.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.wd)
        if args.cuda:
